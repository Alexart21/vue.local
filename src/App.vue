  ...
  <div class="wrapper d-flex justify-content-center">
    ...
    <div class="right">
      <div class="right-inner">
        ...
        <!--  для картинки    -->
        <div
          v-else-if="tmpLayer.type == 'img' && tmpLayer.inUse && !showBgGalery"
        >
          <scale-control :tmpLayer="tmpLayer" />
          <span class="input-title">Координаты</span>
          <div class="d-flex flex-column">
...
export default {
  ...
  methods: {
    ...
    initTxtWidth() {
      // финт ушами для вычесления ширины текстового блока
    ...
    initTxtHeight() {
      // финт ушами для вычесления высоты текстового блока
      let el = document.createElement('span')
      el.style.fontSize = this.defaultFontSize + 'px'
      el.style.fontFamily = this.defaultFontFamily
      el.style.letterSpacing = 0
    ...
    setTxtBoxSize() {
      // устанавливаем размеры txt блока
      let text = this.tmpLayer.content
      let pos = 0
      let lineCount = 1
      while (true) {
        // ищем количество переводов строк в тексте
      ...
      if (lineCount > 1) {
        ...
        arr = text.split('\n')
        arr.forEach(item => {
          // находим самую длинную строку в многострочном тексте
    ...
    getLayers() {
      // достаем если есть сохраненные слои
    ...
    clearTmpLayer() {
      // очистка(возврат к умолчаеию) временного слоя
      this.tmpLayer.id = ''
      this.tmpLayer.class = ''
    ...
    showBgList() {
      // показываем галерею фоновых картинок
